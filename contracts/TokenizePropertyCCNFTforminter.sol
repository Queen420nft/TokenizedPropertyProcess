...
contract TokenizePropertyCCNFTforminter is ERC721, ERC721URIStorage {
...
function mint(address to, uint256 amountNFT) public {
    require(tokenIdCounter.current() + amountNFT <= maxTokenSupply, "NO NFTs");

    mintFrom(to, amountNFT, 0);
}

function mintFrom(address to, uint256 amountNFT, uint256 sourceId) public {
    require(tokenIdCounter.current() + amountNFT <= maxTokenSupply, "NO NFTs");
    ...
    function updateMetaData(uint256 tokenId, uint256 sourceId) public {
        ...
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "', nftName, ' TokenizedProperty",',
